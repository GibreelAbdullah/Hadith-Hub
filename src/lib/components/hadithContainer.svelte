<script lang="ts">
	...
	function capture(i: number, name: string) {
		htmlToImage.toPng(document.getElementById('hadithGroup' + i)!).then(function (dataUrl) {
		...
		<div id="hadithGroup{i}" class="card">
			...
			<div class="metaGroup text-sm p-2 grid">
				<div class="my-4">
					...
					<br />
					Book {@html allHadiths[0].hadiths[i].reference.book}, Hadith {@html allHadiths[0].hadiths[
						i
					].reference.hadith}
				</div>
